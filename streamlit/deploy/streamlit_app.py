...
st.caption("Snowflake + Cortex AI")
...
""", language=None)
    st.divider()
    st.caption("Built with Snowflake")
...
with tab1:
    ...
    st.markdown("Select a claim and run it through **Cortex AI (Claude Sonnet 4.5)** for an instant AI adjuster recommendation.")
    st.divider()
    ...
    if claim_id:
        ...
        with st.expander("**World Bank Market Context** — data fed to Cortex AI", expanded=True):
            st.caption("Sourced from the **Snowflake Marketplace** (World Bank) and joined to each claim at build time. Cortex AI uses these indicators to calibrate risk scores and payout recommendations for the claimant's country.")
        ...
        if existing_decision and str(existing_decision).strip():
            ...
            st.divider()
            re_eval = st.button("Re-evaluate with Cortex AI", type="secondary", use_container_width=True)
        else:
            re_eval = False

        with st.expander("View AI Prompt", expanded=False):
            st.markdown("This is the prompt sent to **Cortex AI (Claude Sonnet 4.5)** by the `EVALUATE_CLAIM` stored procedure:")
...
"recommended_payout":number}}""", language="text")
        ...
        if not existing_decision or not str(existing_decision).strip():
            should_evaluate = st.button("Evaluate with Cortex AI", type="primary", use_container_width=True)
        elif re_eval:
        ...
        if should_evaluate:
            ...
            with st.spinner("Calling Cortex AI (Claude Sonnet 4.5)..."):
                raw = session.sql(f"CALL INSURANCE_DEMO_DB.AI.EVALUATE_CLAIM('{claim_id}')").collect()[0][0]
                elapsed = time.time() - start_ts
            timer_placeholder.caption(f"Cortex AI responded in **{elapsed:.1f}s**")
            result_dict = parse_ai_result(raw)
...
with tab4:
    ...
    try:
        ...
        if not risk_full.empty:
            ...
            st.caption("Combined risk profile used by the AI claim evaluation engine. Cortex AI factors in the claimant's country GDP, insurance maturity, disaster exposure, and displaced population to calibrate risk scores and payout recommendations.")
