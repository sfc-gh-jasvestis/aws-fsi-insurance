...
**Snowflake:** <YOUR_CONNECTION> (<YOUR_SF_ACCOUNT_LOCATOR>) | **Target:** AWS Summit Hong Kong + ASEAN reuse  
**AWS Region:** us-west-2 | **AWS Account:** `$AWS_ACCOUNT_ID` (set `export AWS_ACCOUNT_ID=__AWS_ACCOUNT_ID__` before build)  
...
| 4.2 | `EVALUATE_CLAIM` stored procedure — Snowpark Python calls Amazon Bedrock via SigV4, writes `AI_DECISION`, `AI_REASONING`, and updates `STATUS` (APPROVE→Approved, DENY→Denied, REFER→Under Review). **Full SQL below — run via `snow sql -c <YOUR_CONNECTION>`** |
...
- **Automated deployment:** A `snowflake.yml` config is included in the project root. Deploy with a single command:
  ```
  snow streamlit deploy --replace -c <YOUR_CONNECTION>
  ```
...
- **Data source creation** (after Snowflake user + network policy):
  ...
  ```
  Replace `<ACCOUNT>` with Snowflake account URL prefix (e.g., `your-org-sg-<YOUR_CONNECTION>`) and `<password>` with the user's password.
...
```sql
-- Run in Snowflake (snow sql -c <YOUR_CONNECTION>)
...
- [ ] Set environment variable: `export AWS_ACCOUNT_ID=__AWS_ACCOUNT_ID__` (or your account)
...
- [ ] Snowflake `<YOUR_CONNECTION>` with `ACCOUNTADMIN` role and `CORTEX` warehouse
